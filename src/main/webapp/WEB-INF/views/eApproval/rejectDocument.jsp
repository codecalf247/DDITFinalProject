					...
					<div
						...
						<div class="card-body px-4 py-3">
							<div class="row align-items-center">
								<div class="col-9">
									<h4 class="fw-semibold mb-8">반려함</h4>
									<nav aria-label="breadcrumb">
										<ol class="breadcrumb">
											...
											<li class="breadcrumb-item" aria-current="page">rejectDocument</li>
										</ol>
									</nav>
								</div>
								<div class="col-3"></div>
											...
											<div class="row">
												<div
													...
													<div class="d-flex flex-wrap gap-2 align-items-center">
														<form id="searchForm"
															action="${pageContext.request.contextPath}/eApproval/rejectDocument"
												...
												<div class="table-responsive">
													<table class="table align-middle table-hover">
														...
														<tbody>
															...
															<c:forEach items="${sanctnDocList }" var="rejectSanctnDoc">
																<tr data-sanctnNo="${rejectSanctnDoc.sanctnDocNo}">
																	<td>${rejectSanctnDoc.sanctnDocNo }</td>
																	<td><c:if test="${rejectSanctnDoc.emrgncyYn eq 'Y' }">
																			<span class="mb-1 badge text-bg-danger">긴급</span>
																		</c:if> <a
																		href="${pageContext.request.contextPath }/eApproval/detail/${rejectSanctnDoc.sanctnDocNo}?drafterId=${rejectSanctnDoc.empNo}">
																			${rejectSanctnDoc.sanctnTitle } </a></td>
																	<td><i class="ti ti-user fs-4 me-1"></i>${rejectSanctnDoc.empNm }
																	</td>
																	<td>${rejectSanctnDoc.deptNm }</td>
																	<td>${fn:substring(rejectSanctnDoc.wrtDt, 0, 4) }-
																		${fn:substring(rejectSanctnDoc.wrtDt, 5, 7) }-
																		${fn:substring(rejectSanctnDoc.wrtDt, 8, 10) }</td>
																	<td><div class="mb-1 badge text-bg-danger"
																			id="restore">반려</div></td>
																	<td><c:choose>
																			<c:when test="${rejectSanctnDoc.hasFile eq 'Y' }">
