					...
					<div
						...
						<div class="card-body px-4 py-3">
							<div class="row align-items-center">
								<div class="col-9">
									<h4 class="fw-semibold mb-8">결재대기</h4>
									<nav aria-label="breadcrumb">
										<ol class="breadcrumb">
											...
											<li class="breadcrumb-item" aria-current="page">PendingApproval</li>
										</ol>
									</nav>
								</div>
								<div class="col-3"></div>
											...
											<div class="row">
												<div
													...
													<div class="d-flex flex-wrap gap-2 align-items-center">
														<form id="searchForm"
															action="${pageContext.request.contextPath}/eApproval/pending"
															...
															<c:if test="${empty sanctnDocList}">
																<tr>
																	<td colspan="7"
																		...
																		결재 대기 중인 문서가 없습니다.</td>
																</tr>
															</c:if>
															<c:forEach items="${sanctnDocList }" var="pendingSanctnDoc">
																<tr data-sanctnNo="${pendingSanctnDoc.sanctnDocNo}">
																	<td>${pendingSanctnDoc.sanctnDocNo }</td>
																	<td><c:if
																			test="${pendingSanctnDoc.emrgncyYn eq 'Y' }">
																			<span class="mb-1 badge text-bg-danger">긴급</span>
																		</c:if> <a
																		href="${pageContext.request.contextPath }/eApproval/register/${pendingSanctnDoc.sanctnDocNo}">
																			${pendingSanctnDoc.sanctnTitle } </a></td>
																	<td><i class="ti ti-user fs-4 me-1"></i>${pendingSanctnDoc.empNm }
																	</td>
																	<td>${pendingSanctnDoc.deptNm }</td>
																	<td>${fn:substring(pendingSanctnDoc.wrtDt, 0, 4) }-
																		${fn:substring(pendingSanctnDoc.wrtDt, 5, 7) }-
																		${fn:substring(pendingSanctnDoc.wrtDt, 8, 10) }</td>
																	<td><div class="mb-1 badge text-bg-info"
																			id="restore">대기중</div></td>
																	<td><c:choose>
																			<c:when test="${pendingSanctnDoc.hasFile eq 'Y' }">
