					...
					<div
						...
						<div class="card-body px-4 py-3">
							<div class="row align-items-center">
								<div class="col-9">
									<h4 class="fw-semibold mb-8">결재진행</h4>
									<nav aria-label="breadcrumb">
										<ol class="breadcrumb">
											...
											<li class="breadcrumb-item" aria-current="page">inProcessApproval</li>
										</ol>
									</nav>
								</div>
								<div class="col-3"></div>
											...
											<div class="row">
												<div
													...
													<div class="d-flex flex-wrap gap-2 align-items-center">
														<form id="searchForm"
															action="${pageContext.request.contextPath}/eApproval/inProcess"
															...
															<div class="btn-group">
																<div class="dropdown">
																	<select class="form-select me-2" name="searchType"
																		...
																		<option value="docFormNm"
																			<c:if test="${searchType == 'docFormNm' }">selected</c:if>>양식문서</option>
																	</select>
																</div>
															</div>
												...
												<div class="table-responsive">
													<table class="table align-middle table-hover">
														...
														<tbody>
															<c:if test="${empty inProcessList}">
																<tr>
																	<td colspan="7"
																		style="text-align: center; color: black; font-size: 15px;">
																		결재 진행 중인 문서가 없습니다.</td>
																</tr>
															</c:if>
															<c:forEach items="${inProcessList }" var="inProcessDoc">
																<tr>
																	<td>${inProcessDoc.sanctnDocNo }</td>
																	<td><c:if test="${inProcessDoc.emrgncyYn eq 'Y' }">
																			<span class="mb-1 badge text-bg-danger">긴급</span>
																		</c:if> <a
																		href="${pageContext.request.contextPath }/eApproval/detail/${inProcessDoc.sanctnDocNo}?drafterId=${inProcessDoc.empNo}">
																			${inProcessDoc.sanctnTitle } </a></td>
																	<td><i class="ti ti-user fs-4 me-1"></i>${inProcessDoc.empNm }
																	</td>
																	<td>${inProcessDoc.deptNm }</td>
																	<td>${fn:substring(inProcessDoc.wrtDt, 0, 4) }-
																		${fn:substring(inProcessDoc.wrtDt, 5, 7) }-
																		${fn:substring(inProcessDoc.wrtDt, 8, 10) }</td>
																	<td>
																		<c:if test="${inProcessDoc.docProcessSttus eq '09002'}">
																			<div class="mb-1 badge text-bg-secondary">진행중</div>
																		</c:if>
																	</td>
																	<td><c:choose>
																			<c:when test="${inProcessDoc.hasFile eq 'Y' }">
