										...
										<div class="col-md-9 p-3">
											...
											<div class="row mb-3">
												<div class="col-12 text-end">
													<div
														style="border: 1px solid #ccc; padding: 5px; width: 100px; display: inline-block;">
														<div
															style="font-size: 12px; border-bottom: 1px solid #ccc; padding-bottom: 5px; margin-bottom: 5px;">
															${sanctnDocInfo.jbgdCd }</div>
														<div class="text-center">
															<img id="stampPreview" src="${stampUrl }" alt="도장 이미지"
																class="img-fluid mb-3 rounded-circle"
																style="max-height: 60px;"> <br> <span
																style="font-size: 14px; display: inline-block; margin-top: 5px;">${sanctnDocInfo.empNm }</span>
														</div>
													</div>
												</div>
											</div>
											...
											<div class="form-title mb-1 p-2 bg-light-subtle rounded"
												...
												<label for="docContent" class="form-label fw-bold">문서
													내용</label>
												<button type="button" id="addDummyRow"
													class="btn btn-light btn-sm">더미 데이터</button>
											</div>
											<div class="mx-auto" style="max-width: 700px;">
												<form id="formSubmit">
													<div class="formCn-content mb-4" id="formCn">
														${lastSanctnDocInfo.sanctnCn }</div>
												</form>
										...
										<div class="col-md-3 p-3"
											...
											<div class="card p-3 shadow-sm mt-4">
												...
												<ul class="list-unstyled mb-0">
													<c:forEach items="${reSanctnerCCList }" var="sanctnerCC">
											...
											<c:if test="${not empty lastSanctnDocInfo.fileList }">
												<div class="card p-3 shadow-sm mt-4">
													<h5>첨부파일</h5>
													<div class="form-text mt-0">- 파일 클릭 시 다운로드 됩니다.</div>
													<ul class="list-unstyled mt-2">
														<c:forEach items="${lastSanctnDocInfo.fileList }" var="file">
															<li
...
document.addEventListener("DOMContentLoaded", () => {
	// dom이 만들어진 후, fileGroupNo가 변수에 셋팅된다. (재기안시, 전 기안문서에 파일이 존재할때 파일 그룹번호 설정)
	...
	let fileGroupNo = "${lastSanctnDocInfo.fileGroupNo}";
	...
	document.querySelector("#fileUploadBtn").addEventListener("click", function(){
		...
		// 문서번호도 같이 보내기
		let sanctnDocNo = "${lastSanctnDocInfo.sanctnDocNo }"; 
		formData.append("sanctnDocNo", sanctnDocNo);
		
		console.log("파일:", formData.get("sanctnDocNo"));
		
		fetch("/eApproval/fileUpload?type=re", {
			method : "post",
			body : formData
		}).then((res) => {
			console.log("res:", res);
	...
	// 버튼 연결
	document.querySelector("#addDummyRow").addEventListener("click", addDummyRow);
