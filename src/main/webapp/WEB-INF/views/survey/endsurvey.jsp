					...
					<div
						...
						<div class="card-body px-4 py-3">
							<div class="row align-items-center">
								<div class="col-9">
									<h4 class="fw-semibold mb-8">마감된 설문</h4>
									<nav aria-label="breadcrumb">
										<ol class="breadcrumb">
											...
											<li class="breadcrumb-item" aria-current="page">EndSurvey</li>
										</ol>
									</nav>
								</div>
								<div class="col-3">
											...
											<div class="d-flex align-items-center">
												...
												<div class="ms-auto">
												 <c:if test="${member.empNo eq '202508001'}">
													<a href="${pageContextPath }/main/insertsurvey"
														class="btn btn-sm btn-outline-primary"> 새 설문 작성 </a>
														</c:if>
												</div>
											</div>
											<div class="row px-6">${member.empNm}
												(${member.deptNm})</div>
										</div>
									</div>
								</div>
									...
									<table class="table table-bordered table-hover align-middle">
										...
										<tbody>
											<c:choose>
												<c:when test="${not empty pagingVO.dataList}">
													<c:forEach var="survey" items="${pagingVO.dataList}"
														...
														<tr>
															...
															<td><a
																href="${pageContext.request.contextPath}/main/endsurvey/detail?surveyNo=${survey.surveyNo}">${survey.surveyTitle}</a></td>
															<td>${fn:substring(survey.surveyRegDt, 0, 10)}</td>
															<td>${fn:substring(survey.surveyDdlnDt, 0, 10)}</td>
															<td class="text-center">
																<button class="btn btn-sm btn-outline-danger">마감됨</button>
															</td>
														</tr>
													</c:forEach>
												</c:when>
												<c:otherwise>
													<tr>
														<td colspan="5" class="text-center">마감된 설문이 없습니다.</td>
													</tr>
												</c:otherwise>
											</c:choose>
										</tbody>
        ...
        <nav aria-label="Page navigation" class="m-0">
            <ul class="pagination justify-content-center mb-0">
                ${pagingVO.pagingHTML2} <!-- 실제 li 내용 -->
									...
									<form id="pageForm"
										action="${pageContext.request.contextPath}/main/endsurvey"
										method="get">
