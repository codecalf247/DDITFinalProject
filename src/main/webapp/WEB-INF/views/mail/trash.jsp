              ...
              <ul class="list-group list-group-menu mh-n100">
                ...
                <li class="list-group-item has-submenu">
                  <a class="menu-link" href="#">
                    <i class="ti ti-folder fs-5"></i> 메일함
                ...
                <li class="list-group-item has-submenu mt-2">
                  ...
                  <ul class="submenu">
                    <li class="list-group-item">
                      <a class="menu-link" href="${pageContext.request.contextPath}/mail/temporary"><i class="ti ti-folder fs-5 text-primary me-2"></i>임시보관함</a>
                    </li>
                    <li class="list-group-item">
                      <a class="menu-link" href="${pageContext.request.contextPath}/mail/trash"><i class="ti ti-trash fs-5 text-warning me-2"></i>휴지통</a>
                    </li>
                  </ul>
                </li>
              </ul>
              ...
              <div class="w-100 p-3">
                ...
                <div class="d-flex align-items-center gap-2 mb-2 toolbar-compact">
                  <div class="d-flex gap-2">
                    <button type="button" class="btn btn-sm btn-outline-primary" id="btnRestore">복구</button>
                    <button type="button" class="btn btn-sm btn-outline-danger" id="btnErase">영구삭제</button>
                  </div>
                  ...
                  <div class="ms-auto d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="btn-group">
                      <button id="searchTypeBtn" type="button"
                        ...
                        <c:choose>
                          <c:when test="${searchType eq 'empNm'}">작성자</c:when>
                      ...
                      <ul class="dropdown-menu">
                        ...
                        <li><a class="dropdown-item" data-search-type="empNm" href="#">작성자</a></li>
                    ...
                    <!-- 검색 폼 -->
                    <form id="searchForm"
                          action="${pageContext.request.contextPath}/mail/trash"
                          method="get"
                ...
                <div class="table-responsive">
                  <table class="table table-hover align-middle mb-0" id="trashTable">
                    <thead>
                    <tr>
                    ...
                    <c:if test="${not empty trashList}">
                      <c:forEach items="${trashList}" var="mail">
                        <tr>
                          <td>
                            <input type="checkbox"
                                   class="row-chk"
                                   data-box-no="${mail.boxNo}"
                                   data-email-type="${mail.type}">
                          </td>
                          <td class="text-truncate" style="max-width:220px;"><c:out value="${mail.empNm}"/></td>
                          <td class="text-truncate">
                            <a class="mail-subject" href="${pageContext.request.contextPath}/mail/trash/trashdetail?emailNo=${mail.emailNo}">
                    ...
                    <c:if test="${empty trashList}">
                      <tr><td colspan="4" class="text-center text-muted py-4">휴지통이 비었습니다.</td></tr>
                    </c:if>
                    </tbody>
                  </table>
                </div>
                ...
                <!-- 페이징용 히든 폼 -->
                <form id="pageForm" action="${pageContext.request.contextPath}/mail/trash" method="get">
...
<%@ include file="/module/footerPart.jsp" %>
 ...
 <!-- 체크박스 & 검색/페이징 & 복구/영구삭제 (SweetAlert2 적용 버전) -->
...
$(function () {
  ...
  // ===== SweetAlert 헬퍼 =====
  const swWarn = (text) => Swal.fire({ title: '안내', text, icon: 'warning', confirmButtonText: '확인' });
  const swInfo = (text) => Swal.fire({ title: '안내', text, icon: 'info', confirmButtonText: '확인' });
  const swErr  = (text) => Swal.fire({ title: '오류', text, icon: 'error', confirmButtonText: '확인' });
  const swOk   = (text) => Swal.fire({ title: '완료', text, icon: 'success', confirmButtonText: '확인' });

  // 상위 메뉴 클릭 시 페이지 점프 방지
  ...
  document.querySelectorAll(".dropdown-menu .dropdown-item").forEach(function (item) {
    item.addEventListener("click", function (e) {
      ...
      let val  = this.getAttribute("data-search-type");
      $("#searchTypeBtn").text(text);
      $("#hiddenSearchType").val(val);
      let st = pageForm.find("input[name='searchType']");
      if (st.length) st.val(val);
    });
  });
  ...
  // ===== 선택값 수집 =====
  // (변경) 타입 구분 없이 모든 선택의 box_no 수집
  function pickSelectedIds() {
    return $(".row-chk:checked")
      .map(function () { return $(this).data("box-no"); })
      .get();
  }
  function getAllSelectedData() {
    let data = [];
    $(".row-chk:checked").each(function(){
      data.push({ boxNo: $(this).data("box-no"), type: $(this).data("email-type") });
    });
    return data;
  }

  // ===== 복구 (AJAX JSON + SweetAlert 확인/성공) =====
  $("#btnRestore").on("click", function () {
    let data = getAllSelectedData();
    if (data.length === 0) { swWarn('선택된 메일이 없습니다.'); return; }

    Swal.fire({
      title: '복구 확인',
      text: '선택한 메일을 복구하시겠습니까?',
      icon: 'question',
      ...
      let $btn           = $("#btnRestore").prop("disabled", true);
      let rowCountBefore = $("#trashTable tbody tr").length;
      let selectedCount  = data.length;
      let currentPage    = Number("<c:out value='${pagingVO.currentPage}' default='1'/>");

      $.ajax({
        url: contextPath + "/mail/trash/restore",
        type: "POST",
        data: JSON.stringify(data),
        contentType: "application/json; charset=utf-8",
        beforeSend: function (xhr) { if (csrfToken && csrfHeader) xhr.setRequestHeader(csrfHeader, csrfToken); },
        success: function () {
          swOk('복구가 완료되었습니다.').then(() => {
            let willEmpty = selectedCount >= rowCountBefore;
            let nextPage  = (willEmpty && currentPage > 1) ? currentPage - 1 : currentPage;
            $("#page").val(nextPage);
            $("#pageForm").trigger("submit");
          });
        },
  ...
  // ===== 영구삭제 (폼 전송) – SweetAlert로 안내/확인 =====
  $("#btnErase").on("click", function () {
    const ids = pickSelectedIds(); // (변경) 모든 유형 포함
    if (ids.length === 0) { swWarn('선택된 메일이 없습니다.'); return; }

    Swal.fire({
      title: '영구삭제 확인',
      text: '선택한 메일을 영구 삭제하시겠습니까? (되돌릴 수 없음)',
      icon: 'warning',
      showCancelButton: true,
      ...
      form.method = 'POST';
      form.action = contextPath + '/mail/trash/erase';
      ...
      form.submit(); // 서버에서 flash msg 설정 후 redirect
    });
  });
