        ...
        <div class="flex-grow-1 text-center">
          <div class="btn-group" role="group" aria-label="자료실 카테고리">
            ...
            <a href="${pageContext.request.contextPath}/main/atrashcan"
               class="btn btn-outline-primary active" id="allLibraryBtn">
...
<script>
    ...
    function loadTrashcanContent() {
        fetch('${pageContext.request.contextPath}/main/atrashcanData', {
            method: 'GET'
        })
        .then(response => {
            if (!response.ok) {
    ...
    function permanentlyDeleteFolder(folderNo) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch('${pageContext.request.contextPath}/main/apermanentlyDeleteFolder', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: 'folderNo=' + folderNo
                })
    ...
    function permanentlyDeleteFile(fileNo) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch('${pageContext.request.contextPath}/main/apermanentlyDeleteFile', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: 'fileNo=' + fileNo
                })
